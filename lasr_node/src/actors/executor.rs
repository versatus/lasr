    ...
    pub(super) fn spawn_poll(
        ...
        Ok(tokio::task::spawn(async move {
            ...
            let actor: ActorRef<ExecutorMessage> =
                ractor::registry::where_is(ActorType::RemoteExecutor.to_string())
...
impl ExecutionEngine {
    pub fn new(manager: OciManager, ipfs_client: ipfs_api::IpfsClient) -> Self {
        Self {
            manager,
            ipfs_client,
            handles: HashMap::new(),
    ...
    pub(super) async fn create_bundle(
        ...
        self.manager
            .bundle(&content_id, crate::BaseImage::Bin)
            .await?;
    ...
    pub(super) fn handle_prerequisites(
        &self,
...
impl ExecutorActor {
    ...
    fn execution_success(
        ...
        let actor: ActorRef<EngineMessage> =
    ...
    async fn handle(
        ...
        match message {
            ...
            ExecutorMessage::Create {
                ...
                match state
                    .create_bundle(program_id.to_full_string(), entrypoint, program_args)
                    .await
                {
                    Ok(()) => match self.registration_success(transaction_hash) {
                        Err(e) => {
                ...
                match state.get_program_schema(program_id.to_full_string()) {
                    Ok(schema) => {
                        match schema.get_prerequisites(&op) {
                                ...
                                match state.parse_inputs(&schema, &transaction, op, inputs) {
                                    Ok(inputs) => {
                                            ...
                                            .await
                                        {
                                            Ok(handle) => {
                                                log::info!(
                                                    "result successful, placing handle in handles"
                                                );
                            ...
                            Err(e) => {
                                log::error!(
                                    "Error calling schema.get_prerequisites: executor.rs: 260: {}",
                                    e
                                );
                            }
                        }
                    ...
                    Err(e) => {
                        log::error!(
                            "Error calling state.get_program_schema: executor.rs: 259: {}",
                            e
                        );
                    }
                }
            ...
            ExecutorMessage::Results {
                ...
                match transaction_hash {
                    Some(hash) => match state.handles.remove(&(content_id.clone(), hash.clone())) {
                        Some(handle) => match handle.await {
                            Ok(Ok(output)) => {
                                log::info!("Outputs: {:#?}", &output);
                                match transaction {
                                    Some(tx) => match self.execution_success(&tx, &hash, &output) {
                                        Ok(_) => {
                                            log::info!(
    ...
    async fn handle(
        ...
        match message {
            ExecutorMessage::Retrieve(content_id) => {
                ...
                match state
                    .client
                    .request::<String, &[u8]>("get_object", content_id.as_bytes())
                    .await
                {
                    Ok(_job_id) => {}
                    Err(_e) => {}
            ...
            ExecutorMessage::Exec { transaction } => {
                ...
                match state
                    ...
                    .request::<String, (String, String, String)>(
                        "queue_job",
                        (program_id.to_full_string(), op, inputs),
                    )
                    .await
                {
                            ...
                            Ok(job_id) => {
                                ...
                                match poll_spawn_result {
                                    Ok(handle) => {
                                        // Stash the job_id
                                        let pending_job =
                                            PendingJob::new(handle, tx, transaction_hash);
            ...
            ExecutorMessage::PollJobStatus { job_id } => {
                match state
                    .client
                    .request::<String, &[u8]>("job_status", job_id.to_string().as_bytes())
                    .await
                {
                    Ok(_outputs) => {}
                    Err(_e) => {}
