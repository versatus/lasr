...
use lasr::{ensure_dir_exists, Inputs, OciBundler, OciManager};
use serde_json::json;
...
async fn main() -> Result<(), std::io::Error> {
    ...
    let manager = OciManager::new(bundler);

    let task_manager_1 = manager.clone();
    let py_handle = tokio::task::spawn(async move {
        task_manager_1
            .bundle("testContainerPy", lasr::BaseImage::Bin)
            .await?;
        ...
        let _ = task_manager_1
            .run_container("testContainerPy", None, inputs, None)
            .await?
            .await??;
    ...
    let rs_handle = tokio::task::spawn(async move {
        task_manager_2
            .bundle("testContainerRs", lasr::BaseImage::Bin)
            .await?;
        ...
        let start = std::time::Instant::now();
        let _ = task_manager_2
            .run_container("testContainerRs", None, inputs, None)
            .await?
            .await??;
        let elapsed = start.elapsed();
