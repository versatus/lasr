...
use lasr_compute::{LasrPackageType, LasrPackageBuilder, LasrObjectBuilder, LasrObjectPayloadBuilder, LasrContentType, LasrObjectCid, SignableObject, LasrObject, LasrPackagePayloadBuilder, LasrPackage};
use lasr_types::{
    Address, 
    PayloadBuilder, 
    Account, 
...
use lasr_wallet::{WalletBuilder, Wallet, WalletInfo};
...
fn file_count_arg() -> Arg {
    Arg::new("file-count")
        ...
        .help("the number of files expected, not required can be inferred, but helps for validation")
        .required(false)
        .default_value("0")
...
fn package_path_arg() -> Arg {
    Arg::new("package-path")
        .long("package-path")
        .aliases(["pp", "ppath", "pubpath", "p-path", "pub-path", "pack", "pack-path"])
...
fn publish_command() -> Command {
    Command::new("publish")
        .about(
            "packages, signs, and publishes a package to web3 store."
        )
        .aliases(
            [
            "pub", 
...
async fn handle_publish_command(children: &ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if *wizard {
        ...
        let package_path = children.get_one::<String>("package-path").expect("package path is required"); 
        let ignore_file = children.get_one::<String>("ignore-file");
        let recursive = children.get_one::<bool>("recursive").expect("required or default");
        let file_count = children.get_one::<u32>("file-count").expect("required or default");
        let cids = children.get_one::<Vec<String>>("cids");
        let content_type = children.get_one::<String>("content-type").expect("content type is required");
        let api_version = children.get_one::<u32>("api-version").expect("required or default");
        let annotations_file = children.get_one::<String>("annotations-file");
        let author = children.get_one::<String>("author").expect("required or default");
        let name = children.get_one::<String>("name").expect("required");
        let package_version = children.get_one::<u32>("package-version").expect("required or default");
        let is_local = children.get_one::<bool>("local").expect("required or default");
        ...
        let store = if *is_local { 
            ...
            let addr = remote.expect(
                "remote node is required if not using --local flag"
            ).to_string();
        ...
        package_builder
            ...
            .package_type(LasrPackageType::from((content_type.as_str(), runtime.as_str())))
        ...
        let package = LasrPackage { package_payload, package_sig }; 
...
async fn handle_register_program(children: &ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    let verbose = *children.get_one::<bool>("verbose").expect("required or default");
    if verbose { dbg!("Getting wallet"); }
    let mut wallet = get_wallet(children).await?;
    let address = wallet.address();
    if verbose { println!("Wallet address: {:?}", address); }
    if verbose { dbg!("Getting account"); }
    let cid = children.get_one::<String>("content-id").expect("required");
    let inputs = json!({"contentId": cid});
    if verbose { dbg!("Inputs obtained: {:?}", inputs.to_string()); }
    if verbose { dbg!("Registering program"); }
    let program_address = wallet.register_program(&inputs.to_string()).await.map_err(|e| {
        Box::new(
            std::io::Error::new(
    ...
    if verbose { dbg!("Program registered"); }
    if verbose { dbg!("Getting account again"); }

    println!(
        "{}",
        serde_json::to_string_pretty(
            &serde_json::json!({"program_address": program_address}) 
        ).unwrap()
    );

    wallet.get_account(&address).await;
    if verbose { dbg!("Account obtained again"); }
    Ok(())
}
...
async fn handle_call_command(children: &ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let address = wallet.address();
    let verbose = children.get_one::<bool>("verbose").expect("required or default");
    if *verbose { dbg!("attempting to get account"); }
    wallet.get_account(&address).await;

    if *verbose { dbg!("parsing cli args"); }
...
fn handle_parse_outputs_command(children: &ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(true) = from_file {
        ...
        let outputs: Outputs = serde_json::from_str(&json_str).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
        println!("{:#?}", outputs);
    } else {
...
fn handle_parse_transaction_command(children: &ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    let value = children.ids();
    let json_str = children.get_one::<String>("json").expect("unable to acquire json from command line");
    let transaction: Transaction = serde_json::from_str(&json_str).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
    println!("{:#?}", transaction);
    Ok(())
}
...
fn get_keypair(children: &ArgMatches) -> Result<(SecretKey, PublicKey), Box<dyn std::error::Error>> {
    let values = children.ids();
